...
int MeshTex::setData(Vertex *vertices, GLuint *indices, vector<Texture>textures, int vertSize, int indexSize, bool instanced, int quantity, Shader *shader, Shader *explodeShader, Shader *cloudShader, int startIndex)
{
    currVal = 0;
    maxVal = indexSize / 3;
...
void MeshTex::explosion(mat4 model, mat4 view, mat4 projection, float timeVal)
{
    cloud->drawCloud(model, view, projection, timeVal, true);
    cout << "\n\tCloud drawn.";
    currVal = 0;
    ...
    for (int x = 0; x < chunkCount; x++)
    {
        ...
        chunkLoc = mat4(1.0f);
        vert = vec3(verticesIndexed[currVal].Normal[0], verticesIndexed[currVal].Normal[2], verticesIndexed[currVal].Normal[2]) * timeVal * 0.3f;
    ...
    glBindBufferRange(GL_UNIFORM_BUFFER, 0, VBO[1], 0, indexSize * sizeof(mat4));
