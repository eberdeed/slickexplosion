    ...
    try
    {
        ...
        if (vertSize >= 3)
        {
            ...
            if (hasTex)
            {
                ...
                if (quantity > 0)
                {
                    startIndex = meshTexPtr->setData(vertices, indices, textures, vertSize, indexSize, true, quantity, shader, explodeShader, cloudShader, startIndex);
                }
                else
                {
                    startIndex = meshTexPtr->setData(vertices, indices, textures, vertSize, indexSize, false, -1, shader, explodeShader, cloudShader, startIndex);
                }
            ...
            else
            {
                ...
                if (quantity > 0)
                {
                    startIndex = meshVertPtr->setData(vertices1, indices, colordiff, vertSize, indexSize, true, quantity, shader, explodeShader, cloudShader, startIndex);
                }
                else
                {
                    startIndex = meshVertPtr->setData(vertices1, indices, colordiff, vertSize, indexSize, false, -1, shader, explodeShader, cloudShader, startIndex);
                }
                if (opacity  > 1.0f)
                {
                    opacity = 0.7f;
...
void Model::DrawExplosion(int index, mat4 model, mat4 view, mat4 projection)
{
    timeVal += 1.0f;
    MeshInfo meshItem;
    string type;
    ...
    for (int x = 0; x < limit; x++)
    {
        ...
        meshItem.mesh->explosion(model, view, projection, timeVal);
    }
}
