...
public:
     ...
     * the explosion.
     * startIndex : The first valid buffer location for a texture.  It is passed to this
     * class because the shader will have sampler2D uniforms even thought there are no
     * textures to fill them and they need to be filled with dummy values.
     */
    int setData(Vertex1 *vertices, GLuint *indices, vec3 color, int vertSize, int indexSize, bool instanced, int quantity, Shader *shader, Shader *explodeShader, Shader *cloudShader, int startIndex);
     ...
     * projection is the perspective for which I use the perspective funcition
     * of GLM in the camera class. 
     * model is a vector of the positions and orientations of the mesh 
     * being drawn as the various instances of the object.
     * lights are point lights defined by the PointLight structure.
     * spotLights are spotlights defined by the SpotLight structure.
     * viewPos is the camera position.
     * diffOnly when set to true will provide an image without specular highlights.
